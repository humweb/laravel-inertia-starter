...
export default {
  data() {
    return {
      ...
      isOpen: false,
      hasEnoughSpace: true,
      internalValue: this.modelValue || this.modelValue === 0
        ? deepClone(Array.isArray(this.modelValue) ? this.modelValue : [this.modelValue])
        : []
    };
  },
  props: {
  ...
  methods: {
    ...
    select(option, key) {
      ...
      if (option.isTag) {
        ...
        this.$emit('update:modelValue', this.getValue(), this.id);
